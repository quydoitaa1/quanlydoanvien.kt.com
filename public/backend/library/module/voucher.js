...
/* MAIN VARIABLE */
   ...
   var   $window = $(window),
         $document = $(document),
         $discountNewTimer = $('.discount-new-timer .next-label--switch');
   ...
   HT.changePromotion = () => {
      if($('.discount-button_type').elExists){
         $('.discount-button_type').click(function(){
            ...
            if(radio.val() == 'money' || radio.val() == 'ship' || radio.val() == 'bill'){
            ...
            if(radio.val() == 'bill' || radio.val() == 'ship'){
               $('.discount-object').hide();
            }else{
               console.log(123)
               $('.discount-object').show();
            }
   ...
   HT.initializeFindObject = () => {
      $('.discount-item .hrv-next-input-radio').click(function(){
         let _this = $(this)
   ...
   HT.renderVoucher = () => {
      if($('.renderVoucher').length){
         $('.renderVoucher').click(function(){
            let _this = $(this);
            $('#title').val(HT.renderVoucherCode())
   ...
   HT.renderVoucherCode = (length = 12) => {
      var text = "";
   ...
   HT.switchBillCondition = () => {
      if($('.bill-condition-switch').length){
         $('.bill-condition-switch').click(function(){
            let _this = $(this);
            const value = _this.siblings('input').val()
            switch(value) {
               case 'Giá trị mua tối thiểu':
                  HT.billConditionMinValueHtml(_this)
                  break;
               case 'Số lượng sản phẩm tối thiểu':
                  HT.billConditionMinProductValueHtml(_this)
                  break;
               default:
                  HT.removeBillConditionHtml();
                  break;
            }
         });
      }
   ...
   HT.billConditionClickOnRadio = () => {
      $('.bill-container-item .hrv-next-radio').click(function(){
         let _this = $(this)
   ...
   HT.removeBillConditionHtml = () => {
      if($('.discount-channel--option').length){
         $('.discount-channel--option').remove();
      }
   }

   HT.billConditionMinValueHtml = (object) => {
      let html = '';
      html = html + '<div class="w-50 discount-channel--option mb-4">'
         html = html + '<div class="my-4"><input name="billConditionValue" class="next-input text-left min-width-100px int" placeholder="0 ₫" value=""></div>'
         html = html + '<span class="text-secondary">Áp dụng cho  <span class="text-lowercase">Tất cả đơn hàng</span></span>'
      html = html + '</div>'
      $('.discount-channel--option').remove();
      object.parent().after(html);
   }

   HT.billConditionMinProductValueHtml = (object) => {
      let html = '';
      html = html + '<div class="w-50 discount-channel--option ">'
         html = html + '<div class="my-4"><input name="billConditionValue" class="next-input text-left min-width-100px int" placeholder="" value="0"></div>'
         html = html + '<span class="text-secondary">Áp dụng cho  <span class="text-lowercase">Tất cả sản phẩm</span></span>'
      html = html + '</div>'
      $('.discount-channel--option').remove();
      object.parent().after(html);
   }

   HT.setupVoucherDiscount = () => {
      if($discountNewTimer.length){
         $discountNewTimer.click(function(){
            let _this = $(this)
            let flag = _this.attr('data-flag')
            let nextInput = _this.siblings('input[type=checkbox]')
            if(nextInput.attr('name') != 'allowCoupon' && flag == 0){
               _this.parent().next('.my-4').remove()
               _this.attr('data-flag', 1)
            }else{
               let name = (nextInput.val() == 'countLimit') ? 'countLimitValue' : 'countCustomerValue'
               if(nextInput.attr('name') != 'allowCoupon'){
                  HT.renderHtmlDiscountTimer(_this, name)
               }
               _this.attr('data-flag', 0)
            }
         });
   ...
   HT.renderHtmlDiscountTimer = (object, name) => {
      let html = ''
      html = html + '<div class="my-4 w-50"><input type="number" name="'+name+'" class="next-input text-left min-width-100px int" value="0"></div>'
      object.parent().after(html)
   }
