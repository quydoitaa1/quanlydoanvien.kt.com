...
class Menu extends BaseController{
   ...
   protected $pagination;
	...
	public function __construct(){
		...
		$this->nestedsetbie = new Nestedsetbie(['table' => $this->data['module'],'language' => $this->currentLanguage(), 'foreignkey' => 'objectid']);
	...
	public function index($id = 0, $language = ''){
      if(!$this->authentication->gate('backend.menu.menu.index')){
	...
	public function listmenu($page = 1){
      if(!$this->authentication->gate('backend.menu.menu.listmenu')){
         $this->session->setFlashdata('message-danger', 'Bạn không có quyền truy cập vào chức năng này!');
         return redirect()->to(BASE_URL.route('backend.dashboard.dashboard.index'));
      }
		...
		if($config['total_rows'] > 0){
			...
			$this->data['menuCatalogue'] = $this->AutoloadModel->_get_where([
				'select' => '  tb1.title, tb1.id, tb1.value, tb1.created_at, tb1.value, tb1.userid_created,tb2.fullname as creator',
				'table' => $this->data['module'].'_catalogue as tb1',
				'where' => $where,
				'keyword' => $keyword,
				'join' => [
					[
						'users as tb2','tb1.userid_created = tb2.id','inner'
					]
				],
	...
	public function createmenu($language = ''){
		...
		$object = $this->AutoloadModel->_get_where([
			'select' => '*',
			'table' => 'systems',
			'where' => ['language_id' => $this->currentLanguage()],
		], TRUE);
		$this->data['general']= [];
